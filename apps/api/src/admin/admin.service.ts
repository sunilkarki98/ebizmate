...
import { encrypt, decrypt, maskApiKey } from '@ebizmate/shared';
import OpenAI from 'openai';
import { AiService } from '../ai/ai.service';
import { loadSettings } from '../ai/services/settings';
...
export class AdminService {
    ...
    async logAudit(userId: string, action: string, targetType: string, targetId: string, details?: Record<string, any>) {
        try {
    ...
    async updateWorkspacePlan(adminId: string, workspaceId: string, data: any) {
        await db.update(workspaces)
            .set({
                plan: data.plan,
                status: data.status,
    ...
    async updateAISettings(adminId: string, data: any) {
