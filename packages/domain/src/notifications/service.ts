...
export async function getNotifications(userId: string, limit: number) {
    ...
    const notifications = systemInteractions.map(i => {
        let type: "ingestion" | "escalation" | "order" | "system" = "system";
        ...
        if (i.content?.startsWith("Ingested post:")) {
            ...
            type = "escalation";
            title = "🚨 Bot Stuck";
        } else if (i.content?.startsWith("New Order:") || i.content?.startsWith("Appointment Request:") || i.content?.startsWith("Call Request:")) {
            type = "order";
            title = i.content.includes("Appointment") ? "📅 Appointment Request" : i.content.includes("Call") ? "📞 Call Request" : "🛒 New Order";
        }
