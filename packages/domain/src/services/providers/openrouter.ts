...
export class OpenRouterProvider implements AIProvider {
    ...
    async chat(params: ChatParams): Promise<ChatResult> {
        ...
        if (typeof params.userMessage === "string") {
            ...
            // OpenRouter may not support all multimodal formats — extract text parts
            const textContent = params.userMessage
                .filter(p => p.type === "text")
                .map(p => ("text" in p ? p.text : ""))
                .join(" ");
            messages.push({ role: "user", content: textContent || "[Image content — text extraction not available]" });
        }
